...
export default function App() {
  ...
  return <div className={`app-shell ${sidebarCollapsed?'sidebar-collapsed':''}`}>
    ...
    <main className="main-shell">
      ...
      {view === 'history' && <section className="page-content"><div className="list-toolbar"><div><span className="section-kicker">ARCHIVE</span><h2>全部推荐记录</h2><p>浏览过去推荐的 {history.length} 篇论文，搜索标题并继续阅读或整理。</p></div><label className="search-box"><Search size={17}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索英文或中文标题"/></label></div><div className="paper-list">{filteredHistory.map(paper => <PaperCard compact key={paper.id} paper={paper} defaultLanguage={settings?.default_abstract_language || 'zh'} onLearned={toggleLearned} onNote={openUnifiedNote} onRelated={findRelated} onRepo={discoverRepo} onWiki={startWiki} onRetryAi={retryAI} onChat={setChatPaper} onRead={setReaderPaper}/>)}</div></section>}
...
function SettingsPage({ settings, tags, researchProfiles, onSaved }: { settings: AppSettings; tags: Tag[]; researchProfiles:ResearchProfile[]; onSaved: () => Promise<void> }) {
  ...
  return <section className="page-content settings-page">
    <div className="list-toolbar"><div><span className="section-kicker">PREFERENCES</span><h2>推荐、API 与用量</h2><p>连接常用模型，调整阅读偏好、每日推荐和本机显示方式。</p></div><button className="primary" disabled={saving} onClick={save}>{saving ? <RefreshCw className="spin" size={18}/> : <Check size={18}/>}保存设置</button></div>
