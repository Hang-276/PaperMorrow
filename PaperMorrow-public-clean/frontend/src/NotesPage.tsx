...
export default function NotesPage({focusPaperId,createSignal=0}:{focusPaperId?:number|null;createSignal?:number}) {
  ...
  return <section className={`notes-page ${sidebarOpen ? '' : 'sidebar-collapsed'}`}>
    ...
    <main className="notes-workspace">
      {!active ? <div className="notes-welcome"><div><Braces/></div><span>RESEARCH MEMORY</span><h2>让阅读、推导与灵感留在同一处</h2><p>新建或打开一篇笔记，记录研究过程、插入公式并引用知识库中的论文。</p><button className="primary" onClick={() => setCreating(true)}><Plus/>创建第一篇笔记</button></div> : <>
