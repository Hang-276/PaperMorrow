...
export default function CoworkAgentPanel({group,busy,onStopAgent,onStopAll}:{group:CoworkAgentGroup|null;busy:boolean;onStopAgent:(agent:CoworkAgent)=>void;onStopAll:()=>void}){
  ...
  if(!group)return <section className="cowork-agent-panel empty"><header><Users/><strong>研究团队</strong><span>载入中</span></header><p>正在准备适合当前任务的研究角色。</p></section>
  return <section className="cowork-agent-panel">
    <header><Users/><div><strong>研究团队</strong><small>{active} 个进行中 · {queued} 个待开始 · 最多同时运行 {group.max_parallel} 个</small></div>{group.agents.length===0&&<button className="agent-action" disabled={busy} onClick={onStopAll}><UserPlus/>召集</button>}{queued>0&&<button className="agent-action primary" disabled={busy} onClick={onStopAll}><Play/>运行</button>}{active>0&&queued===0&&<button className="agent-stop-all" disabled={busy} onClick={onStopAll}><Square/>停止全部</button>}</header>
    <div className="agent-supervisor"><span className={`agent-state ${group.supervisor_status}`}><BrainCircuit/>{statusText[group.supervisor_status]}</span><div><strong>任务协调</strong><small>安排研究角色，并把结果、来源和待确认事项汇总到当前任务</small></div></div>
