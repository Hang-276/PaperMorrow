...
```bash
.venv-paper/bin/python -m pip install -e ".[desktop]"
...
推送形如 `v0.5.2` 的 tag 时，工作流会先在两个系统上重新运行后端测试和前端正式构建，再把以下文件上传到 GitHub Releases，并标记为 Beta 预发布版：

- `PaperMorrow-macOS.dmg`
- `PaperMorrow-Windows-Setup.exe`
- `PaperMorrow-Windows-Portable.zip`

这些文件完全托管在 GitHub Release Assets，不需要外部网盘。README 中的下载链接始终指向最新 Release。GitHub-hosted Windows runner 生成的安装包仍需在真实 Windows 10/11 设备上做最终人工检查；未配置 Apple Developer ID 或 Windows 商业签名证书时，系统可能显示安全提示。
